...
function execute_scenario {
    while true; do
        ...
        finished=`grep "score_composed" ${BASE_DIR}/logs_town_$1_route_$2_timely_$3_edet_$4_run_$5/results.json`
        if [[ ! -z "$finished" ]] ; then
            break
        fi
        echo "Deleting incomplete experiment"
        # Deleting the incomplete logs.
        rm -r ${BASE_DIR}/logs_town_$1_route_$2_timely_$3_edet_$4_run_$5/
    done
}
...
function execute_scenario_once {
    ...
    # $6 frame rate
    export RECORD_PATH=${BASE_DIR}/logs_town_$1_route_$2_timely_$3_edet_$4_run_$5/
